...
void print_report(ModelT& res, process_options& op, string mp_type);
...
{  
  ...
  cout << "Computing OCP in soft disk partition function and density in parallel using pipeline TBB library.\n";
  cout << "N = " << n << endl; ;
...
// turn this into a class that takes the functions as parameters...
template <class T> 
void run_calculations(process_options& op, string mp_type)
{
  ...
  Model_density<T> softdisk(n, gammaover2, &unif_T_softdisk);
  if (op.debug) cout << "done\n";
  if (op.debug) cout << "Creating tables of functions to compute the density... ";
  function_set<T> f_dens_tables(op.density_domains, boost::bind(f_dens_softdisk_g<T>,_1,_2,gammaover2), "softdisk", gammaover2, n, mp_type);
  stringstream s_comment_line;
  s_comment_line << "sqrt (pi rho_b)r     rho(r)/rho_b";
  softdisk.initialize_density_set(f_dens_tables, s_comment_line.str());
  if (op.debug) cout << "done\n";
  ...
  model_worker<Model_density<T> > chw(n, &softdisk);
  pipeline.add_filter(chw);
  consolidate_model<Model_density<T> > cons(&softdisk);
  ...
  softdisk.normalize_density(boost::bind(pre_f_softdisk_g<T>, _1, gammaover2), T(gammaover2));
  if (op.debug) cout << "done.\n" << flush;
...
template <class ModelT>
void print_report(ModelT& res, process_options& op, string mp_type) {
  res.print_report(std::cout, mp_type, "softdisk");
